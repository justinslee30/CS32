...
bool City::addFlatulan(int r, int c)
{
	...
	// Don't add a Flatulan at a spot with a player
...
bool City::addPlayer(int r, int c)
{
	...
	// Don't add a player at a spot with a Flatulan
...
void City::preachToFlatulansAroundPlayer()
{
	...
	for (int k = 0; k < m_nFlatulans; )
	{
		...
		// if is orthogonally or diagonally adjacent and conversion succeeds
		if (rowdiff >= -1 && rowdiff <= 1 &&
			coldiff >= -1 && coldiff <= 1 &&
			fp->possiblyGetConverted())
		{
			//when a Flatulan is converted upon being preached to, it's City's history object must be notified about the position it was at when preached to
			m_history.record(fp->row(), fp->col()); //only line changed in City.cpp
			delete m_flatulans[k];
